...
class _MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    var children = <Widget>[
      new InkWell(child: new Padding(child: new Text('Show all contacts'),
    padding: const EdgeInsets.all(16.0),), onTap: () { Navigator.of(context).push(new MaterialPageRoute(builder: (_) => new AllContactsPage()));},),
  new InkWell(child: new Padding(child: new Text('Pick a contact'), padding: const EdgeInsets.all(16.0),),
    onTap: () async {
      Contact contact = await ContactsService.pickContact();
      setState(() {
        _contact = contact;
      });
    },),
      new InkWell(child: new Padding(child: new Text('Export a contact'), padding: const EdgeInsets.all(16.0),),
        onTap: () async {
          await ContactsService.exportContact(
              new Contact(givenName: 'givenname',
                  familyName: 'familyname'));
        },),
      new Padding(child: new Text('Selected contact is:'), padding: const EdgeInsets.all(32.0),)
  ];
    if (_contact != null){
      children.add(new Text(_contact.displayName));
      for (var phone in _contact.phones ?? []){
        children.add(new Text('phone: ${phone.value}'));
      }
    }
    return new Scaffold(
      appBar: new AppBar(title: new Text('Contacts plugin example')),
      body: new Column(children: children, crossAxisAlignment: CrossAxisAlignment.start,
      ),
    );
  }
}
...
class AllContactsPage extends StatefulWidget {
  ...
  @override
  _AllContactsPageState createState() => new _AllContactsPageState();
}
