...
class ContactsService {
  ...
  /**
   * Opens the device phone book page and allow to pick one contact.
   */
  static Future<Contact> pickContact() async{
    var contact = await _channel.invokeMethod('pickContact', null);
    return new Contact.fromMap(contact);
  }

  /**
   * Exports the [contact] to the device contact list by showing it in the device contact edit page.
   */
  static Future exportContact(Contact contact) async {
    print('in export contact --------------');
    print(Contact._toMap(contact));
    await _channel.invokeMethod('exportContact', Contact._toMap(contact));
  print('--------------- done ------------------------');
  }
